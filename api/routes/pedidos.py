...
def tornar_prioritario(cod_pedido: int):
    """Torna um pedido prioritário na fila de preparo."""
    resultado = service.tornar_pedido_prioritario(cod_pedido)
    if not resultado:
        raise HTTPException(
            status_code=400,
            detail="Pedido não encontrado ou não pode ser priorizado",
        )
...
def listar_fila_preparo():
    ...
    Retorna pedidos prioritários primeiro, depois normais.
